...
- 국내 Naver 우선, Yahoo 보조 가격으로 월별 리밸런싱 수량과 금액 계산
...
- 국내 KRX 종목은 Naver Finance 우선 조회, 실패 시 Yahoo `.KS`, `.KQ` 순차 조회
- 해외 종목은 Yahoo Finance 현재가 조회
- 가격 캐시 30초 및 가격 상태 탭의 시세시각/조회시각 구분
- 가격 적용 우선순위: 수동현재가 > 온라인 조회가 > 시트가격
- TIGER/ACE 금 ETF의 개별 고정 목표를 제거하고 계정별 합산 목표 20%를 현재 평가액 비율로 동적 배분
- 금 ETF 합산 목표비중을 Firestore와 Google Sheet 메타데이터에 저장 및 왕복 동기화
...
`fetch_price_snapshot()`은 국내 KRX 티커에 대해 `Naver -> Yahoo -> 시트가격` 순서로 개선됐고 캐시는 30초다. Naver 응답에 거래시각 필드가 있으면 가격 상태 탭에 표시하며, Yahoo 응답은 현재 가격의 실제 체결시각을 안정적으로 확보하지 못해 `확인 불가`로 표시한다. Naver Finance는 비공식 API이므로 응답 형식 변경이나 요청 제한에 대한 운영 확인이 필요하다.
...
4. 장중에 국내 ETF 가격이 Naver 출처로 표시되고 Google Sheet/Yahoo보다 최신인지 실제 값으로 비교한다.
5. Yahoo fallback 가격의 실제 체결시각을 확보할 수 있는지 검토하고 오래된 값 판별 테스트를 추가한다.
6. 금 두 종목의 보유수량을 바꿨을 때 개별 목표는 달라지되 합산 목표가 20%로 유지되는지 배포 앱에서 확인한다.
...
- 국내 KRX 종목은 `Naver -> Yahoo -> 시트가격`, 해외 종목은 `Yahoo -> 시트가격` 순서를 유지한다.
- 사용자가 입력한 수동현재가는 온라인 가격보다 우선한다.
- 금 ETF 두 종목은 개별 목표를 고정하지 않고 합산 목표(기본 20%)만 관리한다. 현재 평가액 비율로 내부 목표를 나누며, 둘 다 미보유일 때만 기존 비율을 초기값으로 쓴다.
...
국내 KRX 가격은 이미 `Naver -> Yahoo -> 시트가격`, 해외 종목은 `Yahoo -> 시트가격` 순서로 개선됐고 캐시는 30초입니다. 장중 실제 가격과 출처를 검증하고 Naver 응답 변경 가능성을 점검하세요. Yahoo fallback은 실제 체결시각이 아직 `확인 불가`이므로 오래된 가격 판별을 추가할 수 있는지 검토하세요.

금 ETF는 TIGER와 ACE의 개별 목표를 고정하지 않고 계정의 합산 목표(기본 20%)를 현재 평가액 비율로 나눕니다. Firestore의 `goldTargetWeight`와 Google Sheet 메타데이터 `금 ETF 합산 목표비중(%)`가 왕복 보존되는지 배포 앱에서 확인하세요. 두 종목 모두 미보유일 때만 기존 비율을 초기 배분값으로 사용합니다.
